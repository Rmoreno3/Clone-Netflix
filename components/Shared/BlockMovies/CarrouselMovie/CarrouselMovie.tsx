...
import { CarrouselMovieProps } from './CarrouselMovie.types';
import {
	Carousel,
	CarouselContent,
	CarouselItem,
...
export function CarrouselMovie(props: CarrouselMovieProps) {
					...
					>
						<Card className="cursor-pointer transition delay-300 group relative">
							<CardContent className="flex aspect-video items-center justify-center p-6 relative border-none rounded-md bg-zinc-900">
								...
								/>
								<div
									className="opacity-0 absolute top-0 transition-all duration-300 z-10 invisible sm:visible delay-300
                w-full bg-zinc-900 rounded-lg scale-0 group-hover:lg:scale-125 group-hover:md:scale-150 group-hover:-translate-y-[5vw] group-hover:opacity-100"
								>
									<Image
										src={movie.thumbnailUrl}
										alt="Movie"
